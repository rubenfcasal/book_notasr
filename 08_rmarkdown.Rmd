...
- Web original del paquete `r cite_pkg(knitr)` [@R-knitr]: [knitr; Elegant, flexible, and fast dynamic report generation with R](https://yihui.name/knitr).
...
Para convertir resultados de R en tablas de una forma simple se puede emplear la función `r cite_fun(ktable, knitr)` del paquete `r cite_pkg(knitr)`. Por ejemplo la Tabla \@ref(tab:kable) se obtuvo mediante el siguiente código:
```{r kable}
knitr::kable(
  head(mtcars), 
...
Otros paquetes proporcionan opciones adicionales: `r cite_pkg(xtable)`, `r cite_cran(stargazer)`, `r cite_pkg(pander)`, `r cite_pkg(tables)` y `r cite_pkg(ascii)`.
...
La mayoría de los campos YAML son opciones que el paquete `r cite_pkg(rmarkdown)` le pasa a Pandoc (ver documentación en el Apéndice \@ref(pandoc)).
...
Para generar un fichero con el código R se puede emplear la función `r cite_fun(ktable, knitr, full = TRUE)`. 
Por ejemplo:
```
purl("Informe.Rmd")
```

Si se quiere además el texto RMarkdown como comentarios tipo `r cite_fun(spin, knitr)`, se puede emplear:
```
...
Una forma rápida de crear este tipo de informes a partir de un fichero de código R es emplear la función `r cite_fun(spin, knitr)` del paquete `r cite_pkg(knitr)` (ver p.e. <https://yihui.org/knitr/demo/stitch/#spin-comment-out-texts>).
...
Para generar el informe se puede emplear la función `r cite_fun(purl, knitr, full = TRUE)`. 
